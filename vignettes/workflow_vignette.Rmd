...
Some of the bioconductor packages used in this workflow changed drastically between different versions, so that newer versions are no longer compatible with tho old ones. Therfore, please make sure your **R version is 4.0 or 4.1** and your **Bioconductor version is 3.11, 3.12 or 3.13**. To install a specific version of bioconductor, install `BiocManager`:
```{r, eval = F}
...
# from Bioconductor
BiocManager::install("scater", version="3.13") # scater v. 1.16.2
BiocManager::install("scran", version="3.13") # scran v. 1.16.0
```

#### Genome annotation (human)
```{r, eval=F}
# ensembldb 2.12.1 and EnsDb.Hsapiens.v86 2.99.0
BiocManager::install(c("ensembldb","EnsDb.Hsapiens.v86"), version="3.13")
# org.Hs.eg.db v. 3.11.4
BiocManager::install("org.Hs.eg.db", version="3.13")
```
...
#### Feature selection
```{r,eval = F}
BiocManager::install("M3Drop", version="3.13")
```
...
BiocManager::install("SC3", version="3.13") # SC3 v. 1.4.2
...
```{r,eval = F}
BiocManager::install("limma", version="3.13") # limma v. 3.32.5
```
...
By setting as_percentage to false, we can also make a similar plot for the absolute raw expression values per gene (I made a custom version for this, because the one in scater has a bug that makes it show the sum of counts instead of mean)
...
```{r}
# for R version 4.0
# qc_stats = scater::quickPerCellQC(sce, percent_subsets="subsets_MT_percent")
# for R version  4.1
qc_stats = quickPerCellQC(sce, sub.fields=T, filter = F)
...
assignment[tsne2 > 10 & tsne1 > 12,cell_type:='NK Cell']
