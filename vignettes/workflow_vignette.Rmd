...
Some of the bioconductor packages used in this workflow changed drastically between different versions, so that newer versions are no longer compatible with tho old ones. Therfore, please make sure your **R version is 3.5.1** and your **Bioconductor version is 3.7**. To install a specific version of bioconductor, run the following:
...
# on Windows, you might need to run this in case devtools does not find Rtools 3.5:
library(devtools)
...
#### Feature selection
```{r,eval = F}
# M3Drop version 3.09.00 (Note: This is still under active development, please let me know if a new version breaks my functions...)
...
## Quality control
We will use scater to calculate various QC metrics. See the [package vignette](https://bioconductor.org/packages/3.7/bioc/vignettes/scater/inst/doc/vignette.html) for details. Then, we use the cyclone function in scran to assign cell cycle phases. Since this takes fairly long for larger datasets, we save the SCESet afterwards so we can just load it and continue the analysis at some later time. 
...
By setting as_percentage to false, we can also make a similar plot for the absolute raw expression values per gene (this looks buggy to me?):

```{r}
p1.2 = plotHighestExprs(sce,feature_names_to_plot = "symbol",as_percentage=F,exprs_values = "counts", colour_cells_by=c("total_features_by_counts"))
...
assignment[tsne2 < -16 & tsne1 > 5,cell_type:='NK Cell']
...
Here, we see that all clusters are highly stable.
...
> Note: For large datasets, the crossvalidation takes a considerable amount of time and should always be run on multiple cores.
...
> Note: The following section (bootstrapping) is computationally expensive, for large datasets, it should be skipped or the number of bootstraps reduced to ~50
...
The main advantage of using mclust is that it is pretty fast and even works on datasets with close to 100000 cells as it does not require calculating distance matrices. Moreover, the cross-validation provides an unsupervised way of determining the number of clusters present in the dataset. However, the downsides are that because the EM procedure is stochastic, outputs differ slightly each time mclust is run. Robustness might be achieved by running the algorithm several times and create a consensus clustering, similar to the procedure used by SC3. Moreover, mclust relies on distributional assumptions, therefore, if the clusters are not gaussian ellipsoids, mclust might not be able to identify them correctly.
...
CellSIUS (*Cell* *S*ubtype *I*dentification from *U*pregulated gene *S*ets ) is a method to find substructure in pre-defined clusters. The underlying idea is to identify cluster-specific correlated gene sets that exhibit a bimodal distribution and then classify cells according to their expression of this gene set. 

The parameters for this algorithm are as follows:

* sce: The SCESet with all our data
* group_id: A column name in colData() that specifies the cell assignment to main clusters. Here, we pretend we only found the 4 major groups and use the manual `cell_type` grouping. 
...
Again, let's make a plot and compare to limma-trend:
...
* vars.to.regress: Character vector (optional). Names of columns in colData(sce) that are possible confounders and whose contribution to the variation should be regressed out.
...
seurat_clustering = function(sce,vars.to.regress=NULL,res=0.6,n_comp=10){
  ...
  if(!is.null(vars.to.regress)){
    if(any(!vars.to.regress%in%names(colData(sce)))){
      stop("Variables to regress out have to be column names in colData(sce)")
    }
    tmp_seurat = AddMetadata(object = tmp_seurat, metadata = colData(sce)[,vars.to.regress])
...
The main function `cellsius_main` takes the following arguments:

* sce: SCESet. Your dataset.
* group_id: Character. A column name in colData(sce) that specifies how the cells should be grouped in clusters.
...
* mcl_path: Path to your MCL installation. The default points to the location used on the Novartis filesystem, for backward compatibility reasons.
...
Functions called by cellsius_main:
```{r}
##################################################
